...
### Testing the models; 
M2_a <- coxph(Surv(entry, exit, event)~1, data=CH)	 
M2_b <- coxph(Surv(entry, exit, event)~as.factor(Year), data=CH)	  
M2_c <- coxph(Surv(entry, exit, event)~as.factor(sex), data=CH)	 
M2_d <- coxph(Surv(entry, exit, event)~as.factor(age), data=CH)
...
Out of `r length(unique(d$organismName))` radio marked rock ptarmigans, `r Deaths[1,2]` were shot by hunters in the study area, and `r Deaths[2,2]` were recorded as dead due to other causes (mostly predation). The birds were marked in February and March, and we estimated the harvest mortality in the during the four weeks of February to be `r 1-round(last(summary(M_h)$surv), 2)` (SE: `r round(last(summary(M_h)$std.err), 2)`).    

Based on cox proportional hazard models, we did not find any signs that of survival differences between years, or between age- and sex categories (AIC for null model: `r round(AIC(M2_a), 2)`; AIC for model including year: `r round(AIC(M2_b), 2)`; AIC for model including sex: `r round(AIC(M2_c), 2)`; AIC for model including age: `r round(AIC(M2_d), 2)`). For all models, the proportional hazards assumption was met (year-model: chi.sq=`r round(prop_test1$table[2], 2)`, p=`r round(prop_test1$table[3], 2)`; sex-model: chi.sq=`r round(prop_test2$table[2], 2)`, p=`r round(prop_test2$table[3], 2)`; age-model: chi.sq=`r round(prop_test3$table[2], 2)`, p=`r round(prop_test3$table[3], 2)`). Based on the pooled sample, probability to survive from February 1st and through July was estimated at `r round(last(summary(M1)$surv), 2)` (SE: `r round(last(summary(M1)$std.err), 2)`) (Figure 2). Survival probability for late winter to start of the breeding season (February 1st - April 30th) was estimated at `r round(last(summary(M_e)$surv), 2)` (SE: `r round(last(summary(M_e)$std.err), 2)`), whereas survival probability for the breeding season (May 1st -July 31st) was estimated at `r round(last(summary(M_l)$surv), 2)` (SE: `r round(last(summary(M_l)$std.err), 2)`).    
...
*Figure 1:* Map of the study area, with Norway marked in orange and the study area marked with a red circle. 

*Figure 2:* Kaplan-Meyer survival curve for a sample of rock ptarmigans radio collared in Central Norway in 2012-2013. The time on the x-axis is shifted so that week 1 represent the first week in February each year, whereas week 31 is the last week in July. Lower table depicts the number at risk throughout the study period. 

*Figure 3:* Linear displacement from the capture site plotted agains week. Each line represent the trajectory for one individual bird, and red crosses indicates death events.  
