...
d_Morph <- as_tibble(read_delim("data/measurementorfact.txt", delim="\t", quote = "")) %>% 
           ...
           left_join(., d) %>% 
           select(organismID, organismName, occurrenceID, measurementValue) %>%
...
temp <- as_tibble(AICcmodavg::aictab(M_list, modnames=c("Intercept only", "S(year)", "S(sex)", "S(age)", "S(weight)", "S(weight + sex)", "S(weight + age"))) %>% select(-LL, -ModelLik)
